"""
TITAN PRODUCTION BOT v2.0
=========================
Enhanced with Professional Trading Book Concepts

New Features:
- Support/Resistance detection
- ATR-based trailing stops
- News time blacklist
- Gap trading
- ADX trend filter
- Enhanced volume analysis

Based on analysis of 4 professional trading books.
"""

import sys, os
sys.path.insert(0, os.path.dirname(os.path.dirname(os.path.abspath(__file__))))

import MetaTrader5 as mt5
import pandas as pd
import numpy as np
import time
import logging
import sqlite3
from datetime import datetime, timezone
from typing import Dict, List, Optional, Tuple
from dataclasses import dataclass

# Import our best components
from titan_system.agents.execution_decision_agent import ExecutionDecisionAgent, QuantSignal, Direction
from titan_system.core.circuit_breaker import CircuitBreaker
from titan_system.execution.mt5_executor import MT5Executor

# Logging
logging.basicConfig(
    level=logging.INFO,
    format='%(asctime)s [%(levelname)s] %(message)s',
    handlers=[
        logging.FileHandler("titan_production_v2.log", encoding='utf-8'),
        logging.StreamHandler()
    ]
)
logger = logging.getLogger("TitanV2")


@dataclass
class TradeRecord:
    """Record of a trade for logging"""
    timestamp: datetime
    symbol: str
    direction: str
    entry_price: float
    lot_size: float
    stop_loss: float
    take_profit: float
    score: float
    reason: str
    ticket: Optional[int] = None


class TitanProductionV2:
    """
    Enhanced Production Bot with Trading Book Concepts
    """
    
    VERSION = "2.0.0"
    
    # Curated symbols
    UNIVERSE = [
        "EURUSD", "GBPUSD", "USDJPY", "AUDUSD", "USDCAD",
        "GOLD", "BTCUSD", "US500"
    ]
    
    # Correlation groups
    CORRELATION_GROUPS = {
        "USD": ["EURUSD", "GBPUSD", "USDJPY", "AUDUSD", "USDCAD"],
        "JPY": ["USDJPY", "EURJPY", "GBPJPY"],
        "GOLD": ["GOLD", "XAUUSD"],
        "CRYPTO": ["BTCUSD", "ETHUSD"]
    }
    
    # News blacklist (major events - UTC times)
    NEWS_BLACKOUT = [
        (13, 30),  # NFP, FOMC (1:30 PM UTC)
        (14, 30),  # FED announcements
    ]
    
    def __init__(self):
        # Core components
        self.executor = MT5Executor()
        self.execution_agent = ExecutionDecisionAgent(trading_mode="LIVE")
        self.circuit_breaker = CircuitBreaker(max_daily_loss_percent=5.0)
        
        # Configuration
        self.risk_per_trade = 0.5
        self.max_total_risk = 5.0
        self.max_positions = 8
        self.max_per_group = 2
        self.scan_interval = 120
        
        # State
        self.db_conn = None
        self.running = False
        self.cycle_count = 0
        self.session_start_equity = 0.0
        self.consecutive_losses = 0
        
        logger.info(f"🚀 Titan Production Bot v{self.VERSION} - Enhanced Edition")
    
   def start(self):
        """Start the enhanced production bot"""
        logger.info("="*60)
        logger.info("TITAN PRODUCTION V2 - ENHANCED WITH TRADING BOOKS")
        logger.info("="*60)
        
        if not self.executor.connect():
            logger.critical("MT5 connection failed!")
            return
        
        self._init_database()
        
        account = self.executor.get_account_info()
        self.session_start_equity = account.get('equity', 0)
        
        logger.info(f"Account: {account.get('login')}")
        logger.info(f"Equity: ${self.session_start_equity:,.2f}")
        logger.info(f"Features: S/R, ATR Stops, News Filter, Gap Trading")
        logger.info("="*60)
        
        self.running = True
        
        while self.running:
            try:
                self._cycle()
                time.sleep(self.scan_interval)
            except KeyboardInterrupt:
                logger.info("Shutdown requested...")
                break
            except Exception as e:
                logger.error(f"Cycle error: {e}", exc_info=True)
                time.sleep(60)
        
        self._shutdown()
    
    def _cycle(self):
        """Enhanced cycle with all features"""
        self.cycle_count += 1
        logger.info(f"--- CYCLE {self.cycle_count} ---")
        
        # 1. Three strikes rule
        if self.consecutive_losses >= 3:
            logger.warning("⚠️ 3 STRIKES - Paused for 1 hour")
            time.sleep(3600)
            self.consecutive_losses = 0
            return
        
        # 2. Circuit breaker
        account = self.executor.get_account_info()
        safe, reason = self.circuit_breaker.check_safe_to_trade(account)
        
        if not safe:
            logger.warning(f"⛔ Trading halted: {reason}")
            self._manage_positions()
            return
        
        # 3. Manage positions
        self._manage_positions()
        
        # 4. Check position limits
        positions = mt5.positions_get()
        current_count = len(positions) if positions else 0
        
        if current_count >= self.max_positions:
            logger.info(f"Max positions ({self.max_positions}) reached")
            return
        
        # 5. Scan for opportunities
        for symbol in self.UNIVERSE:
            if current_count >= self.max_positions:
                break
            
            if not self._check_correlation_limit(symbol):
                continue
            
            if self._has_position(symbol):
                continue
            
            # Try regular signal
            signal = self._analyze(symbol)
            if signal and signal.score >= 85:
                if self._execute(signal):
                    current_count += 1
                    continue
            
            # Try gap signal (Monday mornings)
            gap_signal = self._scan_for_gaps(symbol)
            if gap_signal:
                if self._execute(gap_signal):
                    current_count += 1
    
    def _is_power_hour(self) -> Tuple[bool, str]:
        """Session filter from daytrading book"""
        utc_hour = datetime.now(timezone.utc).hour
        
        if 7 <= utc_hour < 10:
            return True, "LONDON_OPEN"
        if 12 <= utc_hour < 13:
            return True, "OVERLAP"
        if 13 <= utc_hour < 16:
            return True, "NY_OPEN"
        if 17 <= utc_hour < 20:
            return False, "LUNCH_DEAD"
        if 21 <= utc_hour or utc_hour < 5:
            return False, "AFTER_HOURS"
        
        return True, "OK"
    
    def _is_news_time(self) -> bool:
        """Avoid trading 30 mins around major news"""
        utc_time = datetime.now(timezone.utc)
        hour, minute = utc_time.hour, utc_time.minute
        
        for news_hour, news_min in self.NEWS_BLACKOUT:
            if hour == news_hour and abs(minute - news_min) < 30:
                return True
        
        return False
    
    def _find_sr_levels(self, df: pd.DataFrame) -> Tuple[List[float], List[float]]:
        """Find Support/Resistance levels from pivot points"""
        lookback = min(50, len(df) - 10)
        
        pivot_highs = []
        pivot_lows = []
        
        for i in range(5, len(df)-5):
            # Pivot high
            if df['high'].iloc[i] == df['high'].iloc[i-5:i+6].max():
                pivot_highs.append(df['high'].iloc[i])
            
            # Pivot low
            if df['low'].iloc[i] == df['low'].iloc[i-5:i+6].min():
                pivot_lows.append(df['low'].iloc[i])
        
        # Cluster nearby levels (within 0.1%)
        resistance = self._cluster_levels(pivot_highs)
        support = self._cluster_levels(pivot_lows)
        
        return support, resistance
    
    def _cluster_levels(self, levels: List[float], tolerance=0.001) -> List[float]:
        """Group nearby levels together"""
        if not levels:
            return []
        
        levels = sorted(levels)
        clustered = []
        current_cluster = [levels[0]]
        
        for level in levels[1:]:
            if abs(level - current_cluster[-1]) / level < tolerance:
                current_cluster.append(level)
            else:
                clustered.append(np.mean(current_cluster))
                current_cluster = [level]
        
        clustered.append(np.mean(current_cluster))
        return clustered
    
    def _too_close_to_sr(self, price: float, sr_levels: List[float], buffer=0.002) -> bool:
        """Check if price too close to S/R (avoid false breakouts)"""
        for level in sr_levels:
            if abs(price - level) / price < buffer:
                return True
        return False
    
    def _calculate_atr(self, df: pd.DataFrame, period=14) -> float:
        """Calculate ATR for dynamic stops"""
        high = df['high']
        low = df['low']
        close = df['close']
        
        tr1 = high - low
        tr2 = abs(high - close.shift())
        tr3 = abs(low - close.shift())
        
        tr = pd.concat([tr1, tr2, tr3], axis=1).max(axis=1)
        atr = tr.rolling(period).mean()
        
        return atr.iloc[-1]
    
    def _calculate_adx(self, df: pd.DataFrame, period=14) -> float:
        """Calculate ADX for trend strength"""
        high = df['high']
        low = df['low']
        close = df['close']
        
        plus_dm = high.diff()
        minus_dm = -low.diff()
        
        plus_dm[plus_dm < 0] = 0
        minus_dm[minus_dm < 0] = 0
        
        tr1 = high - low
        tr2 = abs(high - close.shift())
        tr3 = abs(low - close.shift())
        tr = pd.concat([tr1, tr2, tr3], axis=1).max(axis=1)
        
        atr = tr.rolling(period).mean()
        
        plus_di = 100 * (plus_dm.rolling(period).mean() / atr)
        minus_di = 100 * (minus_dm.rolling(period).mean() / atr)
        
        dx = 100 * abs(plus_di - minus_di) / (plus_di + minus_di)
        adx = dx.rolling(period).mean()
        
        return adx.iloc[-1]
    
    def _scan_for_gaps(self, symbol: str) -> Optional[QuantSignal]:
        """Monday gap trading strategy (80% fill rate)"""
        day_of_week = datetime.now(timezone.utc).weekday()
        
        # Only scan on Monday (0) or after weekend
        if day_of_week not in [0, 6]:
            return None
        
        try:
            rates = mt5.copy_rates_from_pos(symbol, mt5.TIMEFRAME_H4, 0, 5)
            if rates is None or len(rates) < 2:
                return None
            
            df = pd.DataFrame(rates)
            
            prev_close = df.iloc[-2]['close']
            curr_open = df.iloc[-1]['open']
            
            gap_pct = (curr_open - prev_close) / prev_close * 100
            
            if abs(gap_pct) > 0.5:  # Significant gap
                if gap_pct > 0:
                    # Gap up - expect fill
                    return QuantSignal(
                        symbol=symbol,
                        direction=Direction.SELL,
                        score=88,
                        setup_type="GAP_FILL",
                        entry_price=curr_open,
                        stop_loss=curr_open * 1.005,
                        take_profit=prev_close,
                        expected_r=2.0,
                        reasoning=[f"Gap fill: {gap_pct:.1f}% up", "80% probability"]
                    )
                else:
                    # Gap down - expect fill
                    return QuantSignal(
                        symbol=symbol,
                        direction=Direction.BUY,
                        score=88,
                        setup_type="GAP_FILL",
                        entry_price=curr_open,
                        stop_loss=curr_open * 0.995,
                        take_profit=prev_close,
                        expected_r=2.0,
                        reasoning=[f"Gap fill: {gap_pct:.1f}% down", "80% probability"]
                    )
        except:
            pass
        
        return None
    
    def _analyze(self, symbol: str) -> Optional[QuantSignal]:
        """Enhanced analysis with all trading book concepts"""
        try:
            # News filter
            if self._is_news_time():
                logger.debug(f"{symbol} - News blackout")
                return None
            
            # Session filter
            is_good_time, session = self._is_power_hour()
            if not is_good_time:
                logger.debug(f"{symbol} - Bad session: {session}")
                return None
            
            # Get data
            rates = mt5.copy_rates_from_pos(symbol, mt5.TIMEFRAME_M15, 0, 100)
            if rates is None or len(rates) < 50:
                return None
            
            df = pd.DataFrame(rates)
            
            # Calculate indicators
            df['EMA9'] = df['close'].ewm(span=9).mean()
            df['EMA21'] = df['close'].ewm(span=21).mean()
            
            delta = df['close'].diff()
            gain = (delta.where(delta > 0, 0)).rolling(14).mean()
            loss = (-delta.where(delta < 0, 0)).rolling(14).mean()
            df['RSI'] = 100 - (100 / (1 + gain / loss.replace(0, 0.0001)))
            
            df['MOM'] = df['close'].pct_change(5) * 100
            
            # Volume analysis
            df['VOL_MA'] = df['tick_volume'].rolling(20).mean()
            df['VOL_RATIO'] = df['tick_volume'] / df['VOL_MA'].replace(0, 1.0)
            
            curr = df.iloc[-1]
            prev = df.iloc[-2]
            
            # Volume filter
            if curr['VOL_RATIO'] < 0.8:
                logger.debug(f"{symbol} - Low volume: {curr['VOL_RATIO']:.2f}x")
                return None
            
            # S/R detection
            support, resistance = self._find_sr_levels(df)
            all_sr = support + resistance
            
            if self._too_close_to_sr(curr['close'], all_sr):
                logger.debug(f"{symbol} - Too close to S/R")
                return None
            
            # ADX trend filter
            adx = self._calculate_adx(df)
            if adx < 20:
                logger.debug(f"{symbol} - Weak trend (ADX {adx:.0f})")
                return None
            
            # ATR for dynamic stops
            atr = self._calculate_atr(df)
            
            # Signal detection
            direction = None
            score = 0
            reason = []
            
            volume_boost = 5 if curr['VOL_RATIO'] > 1.5 else 0
            adx_boost = 10 if adx > 30 else 0
            
            if curr['RSI'] < 15:
                direction = Direction.BUY
                score = 95 + volume_boost + adx_boost
                reason = [f"RSI extreme oversold ({curr['RSI']:.0f})", f"Session: {session}"]
            elif curr['RSI'] > 85:
                direction = Direction.SELL
                score = 95 + volume_boost + adx_boost
                reason = [f"RSI extreme overbought ({curr['RSI']:.0f})", f"Session: {session}"]
            elif curr['RSI'] < 25 and curr['MOM'] > 1.0:
                direction = Direction.BUY
                score = 90 + volume_boost + adx_boost
                reason = [f"RSI oversold + momentum"]
            elif curr['RSI'] > 75 and curr['MOM'] < -1.0:
                direction = Direction.SELL
                score = 90 + volume_boost + adx_boost
                reason = [f"RSI overbought + momentum"]
            elif prev['EMA9'] <= prev['EMA21'] and curr['EMA9'] > curr['EMA21'] and curr['MOM'] > 1.0 and adx > 25:
                direction = Direction.BUY
                score = 88 + volume_boost
                reason = ["EMA bullish cross + strong trend"]
            elif prev['EMA9'] >= prev['EMA21'] and curr['EMA9'] < curr['EMA21'] and curr['MOM'] < -1.0 and adx > 25:
                direction = Direction.SELL
                score = 88 + volume_boost
                reason = ["EMA bearish cross + strong trend"]
            
            if not direction:
                return None
            
            # Add volume/ADX to reason
            if curr['VOL_RATIO'] > 1.5:
                reason.append(f"High volume ({curr['VOL_RATIO']:.1f}x)")
            if adx > 30:
                reason.append(f"Strong trend (ADX {adx:.0f})")
            
            # ATR-based stops
            current_price = curr['close']
            info = mt5.symbol_info(symbol)
            point = info.point
            
            # Use ATR for stop distance (2x ATR)
            sl_distance = atr * 2
            
            if direction == Direction.BUY:
                stop_loss = current_price - sl_distance
                take_profit = current_price + (sl_distance * 3)  # 1:3 RR with ATR
            else:
                stop_loss = current_price + sl_distance
                take_profit = current_price - (sl_distance * 3)
            
            return QuantSignal(
                symbol=symbol,
                direction=direction,
                score=score,
                setup_type="ENHANCED",
                entry_price=current_price,
                stop_loss=stop_loss,
                take_profit=take_profit,
                expected_r=3.0,
                reasoning=reason
            )
            
        except Exception as e:
            logger.warning(f"Analysis error for {symbol}: {e}")
            return None
    
    # [Rest of methods: execute, manage_positions, etc. - same as before]
    # ... (keeping existing methods)
    
    def _execute(self, signal: QuantSignal) -> bool:
        """Execute trade"""
        try:
            logger.info(f"SIGNAL: {signal.symbol} {signal.direction.value} | Score: {signal.score}")
            logger.info(f"Reason: {', '.join(signal.reasoning)}")
            
            account = self.executor.get_account_info()
            equity = account['equity']
            risk_amount = equity * (self.risk_per_trade / 100)
            
            info = mt5.symbol_info(signal.symbol)
            point = info.point
            tick_value = info.trade_tick_value if info.trade_tick_value > 0 else 1.0
            
            sl_distance = abs(signal.entry_price - signal.stop_loss) / point
            lot = risk_amount / (sl_distance * tick_value)
            lot = max(info.volume_min, min(info.volume_max, round(lot, 2)))
            
            sl_points = int(sl_distance)
            tp_points = int(abs(signal.take_profit - signal.entry_price) / point)
            
            result = self.executor.execute_order(
                symbol=signal.symbol,
                order_type=signal.direction.value,
                lot=lot,
                sl_points=sl_points,
                tp_points=tp_points,
                comment=f"TitanV2: {signal.score:.0f}"
            )
            
            if result:
                self._log_trade(TradeRecord(
                    timestamp=datetime.now(timezone.utc),
                    symbol=signal.symbol,
                    direction=signal.direction.value,
                    entry_price=signal.entry_price,
                    lot_size=lot,
                    stop_loss=signal.stop_loss,
                    take_profit=signal.take_profit,
                    score=signal.score,
                    reason=', '.join(signal.reasoning),
                    ticket=result.get('ticket')
                ))
                
                logger.info(f"✅ Executed: {signal.symbol} {signal.direction.value} {lot} lots")
                
                # Reset consecutive losses on successful execution
                # (will be updated when trade closes)
                
                return True
            else:
                logger.error(f"❌ Execution failed: {signal.symbol}")
                return False
                
        except Exception as e:
            logger.error(f"Execute error: {e}", exc_info=True)
            return False
    
    def _manage_positions(self):
        """Auto break-even for profitable positions"""
        positions = mt5.positions_get()
        if not positions:
            return
        
        for pos in positions:
            if "TitanV2" not in pos.comment and "Titan" not in pos.comment:
                continue
            
            profit_per_lot = pos.profit / pos.volume if pos.volume > 0 else 0
            
            if profit_per_lot > 100:
                entry = pos.price_open
                sl = pos.sl
                
                if pos.type == 0:  # BUY
                    if sl < entry:
                        self._move_to_breakeven(pos)
                else:  # SELL
                    if sl > entry or sl == 0:
                        self._move_to_breakeven(pos)
    
    def _move_to_breakeven(self, pos):
        """Move SL to break-even"""
        entry = pos.price_open
        
        request = {
            "action": mt5.TRADE_ACTION_SLTP,
            "symbol": pos.symbol,
            "position": pos.ticket,
            "sl": entry,
            "tp": pos.tp,
        }
        
        result = mt5.order_send(request)
        if result.retcode == mt5.TRADE_RETCODE_DONE:
            logger.info(f"🛡️  Break-even: {pos.symbol} @ {entry:.5f}")
    
    def _check_correlation_limit(self, symbol: str) -> bool:
        """Check correlation limits"""
        positions = mt5.positions_get()
        if not positions:
            return True
        
        group_counts = {}
        for pos in positions:
            for group, symbols in self.CORRELATION_GROUPS.items():
                if any(s in pos.symbol for s in symbols):
                    group_counts[group] = group_counts.get(group, 0) + 1
        
        for group, symbols in self.CORRELATION_GROUPS.items():
            if any(s in symbol for s in symbols):
                current = group_counts.get(group, 0)
                if current >= self.max_per_group:
                    return False
        
        return True
    
    def _has_position(self, symbol: str) -> bool:
        """Check if already have position"""
        positions = mt5.positions_get(symbol=symbol)
        return positions is not None and len(positions) > 0
    
    def _init_database(self):
        """Initialize database"""
        self.db_conn = sqlite3.connect("titan_v2.db")
        cursor = self.db_conn.cursor()
        
        cursor.execute("""
            CREATE TABLE IF NOT EXISTS trades (
                id INTEGER PRIMARY KEY AUTOINCREMENT,
                timestamp TEXT NOT NULL,
                symbol TEXT NOT NULL,
                direction TEXT NOT NULL,
                entry_price REAL NOT NULL,
                lot_size REAL NOT NULL,
                stop_loss REAL NOT NULL,
                take_profit REAL NOT NULL,
                score REAL NOT NULL,
                reason TEXT,
                ticket INTEGER
            )
        """)
        
        self.db_conn.commit()
        logger.info("✅ Database initialized")
    
    def _log_trade(self, trade: TradeRecord):
        """Log trade to database"""
        cursor = self.db_conn.cursor()
        cursor.execute("""
            INSERT INTO trades (timestamp, symbol, direction, entry_price, lot_size,
                              stop_loss, take_profit, score, reason, ticket)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        """, (
            trade.timestamp.isoformat(),
            trade.symbol,
            trade.direction,
            trade.entry_price,
            trade.lot_size,
            trade.stop_loss,
            trade.take_profit,
            trade.score,
            trade.reason,
            trade.ticket
        ))
        self.db_conn.commit()
    
    def _shutdown(self):
        """Shutdown"""
        logger.info("Shutting down...")
        
        self.running = False
        
        if self.db_conn:
            self.db_conn.close()
        
        self.executor.shutdown()
        
        logger.info(f"📊 Total cycles: {self.cycle_count}")
        logger.info("Goodbye!")


if __name__ == "__main__":
    bot = TitanProductionV2()
    bot.start()
